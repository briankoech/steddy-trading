...
export function Thumbnail({ file, alt, className }: any) {
  ...
  return (
    <>
      {
        image ?
          <GatsbyImage image={image} alt={alt} className={`block w-full h-72 ${className}`} />
          :
          <StaticImage src='https://placehold.co/265/png' alt={alt} className={`block w-full h-72 ${className}`} />
      }
